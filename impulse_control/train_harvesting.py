...
from . import harvesting as harvesting_module
...
def _validate_resume_result(
    ...
    profile = published_training_parameters("harvesting", "unlimited", dimension)
    if not np.isclose(float(cfg.time.T), expected_horizon):
        raise RuntimeError("Resume checkpoint has a different harvesting horizon.")
    expected_profile = (
        (16, 2, 16, 8, 1)
        if smoke_test
        else (
    ...
    if actual_profile != expected_profile or cfg.opt.min_rel_impulse is not None:
        raise RuntimeError("Resume checkpoint has a different harvesting training profile.")
    if not smoke_test and not np.isclose(float(cfg.net.transfer_lr), profile["transfer_lr"]):
        raise RuntimeError("Resume checkpoint has a different harvesting transfer rate.")
    if not np.isclose(float(cfg.design.x_max), 2.0):
        raise RuntimeError("Resume checkpoint has a different harvesting design domain.")
    ...
    for name, target in expected.items():
        ...
        if not np.isclose(scalar, target):
            raise RuntimeError(f"Resume checkpoint has a different harvesting {name}.")
...
def train_limited(
    ...
    published = published_training_parameters("harvesting", "limited", state_dim)
...
def train_unlimited(
    ...
    transfer_steps = 500                                        # Transfer learning iterations
    batch_size = 8192                                           # Training batch size
    published = published_training_parameters("harvesting", "unlimited", d_state)
    n_global = int(published["randomized_candidates"])          # Random impulse candidates
    n_global_batch = 512                                        # Candidate batch size
    N_k = int(published["design_states"])                       # Regression states per date
    M_k = int(published["rollouts_per_state"])                  # Rollouts averaged per state
    ...
    if not smoke_test and horizons is None and completed_horizons == set(PUBLISHED_REPORTING_HORIZONS):
        print(f"Published harvesting checkpoint already complete: {output}")
        return
    ...
    if horizons is None:
        ...
        if len(all_results) != 1 or float(all_results[0]["T"]) != source_horizon:
            raise RuntimeError("The harvesting checkpoint requires one source model.")
